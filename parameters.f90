...
module parameter_values
  ...
  character(100), parameter, public ::  foldername = "./"
  ...
  real(kind=RP), parameter, public :: forward_length_max = 30._RP                                       !Max length for forward, used to avoid weird behaviour right before they reach length_max
  integer,       parameter, public :: R_categories = 22                                                 !Energy reserves categories
  real(kind=RP), parameter, public :: THF_min = 0._RP, THF_max = 5._RP, THF_step = 0.25_RP              !THF concentration in serum [ng ml-1] (Tagawa et al., 1994; Varghese & Oommen, 1999; Varghese et al., 2001; J. Geoffrey Eales, 1988)
  integer,       parameter, public :: Th_categories = nint((THF_max-THF_min)/THF_step)+1                !Thyroid Hormone Function categories
  real(kind=RP), parameter, public :: GHF_min = 0._RP, GHF_max = 200._RP, GHF_step = 1.25_RP            !GHF concentration in serum [ng ml-1] (Cameron et al., 2007; Uchida et al., 2003)
  integer,       parameter, public :: Gh_categories = nint((GHF_max-GHF_min)/GHF_step)+1                !Growth Hormone Function categories
  real(kind=RP), parameter, public :: OXF_min = 0._RP, OXF_max = 2500._RP, OXF_step = 37.5_RP           !OXF concentration in blood serum [pg ml-1] (from pigs, humans) (Tomasik et al., 2004; Kaminski et al., 2013)
  integer,       parameter, public :: Ox_categories = nint((OXF_max-OXF_min)/OXF_step)+1                !Orexin Hormone Function categories
  integer,       parameter, public :: t_max = 250                                                       !Number of timesteps
  real(kind=RP), parameter, public :: t_duration = 7._RP                                                !Days in each timestep
  integer,       parameter, public :: E_categories = 12                                                 !Number of food environment categories
  ...
  real(kind=RP), parameter, public :: SMR_exp = 0.7_RP                                   !SMR scales proportional to W^SMR_Exp (Killen et al., 2007)
  ...
  real(kind=RP), parameter, public :: conversion_efficiency_reserves = 0.85_RP           !Conversion efficiency for intake to reserves and from reserves pay for metabolism and fixed costs
  ...
  real(kind=RP), parameter, public :: starvation_limit = 0._RP                           !When reserves fall below this proportion of reserves*max the survival approaches 0
  ...
  real(kind=RP), parameter, public :: ValueOfE_min = 1._RP - 0.64_RP, ValueOfE_max = 1._RP + 0.64_RP !The difference between the worst and the best environment
  ...
  !Mortality
  real(kind=RP), parameter, public :: M_size_coeff = 1.3_RP*(t_duration/365._RP)          !Size-dependent mortality
