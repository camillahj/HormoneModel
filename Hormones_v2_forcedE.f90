...
! - All individuals are run though the same environment
! - Individuals are forced through environments after length 20
...
program Hormones_v2
  ...
  integer, parameter       :: EX_n = 3 !Number of individuals uses for experiments
!~   real(kind=RP), parameter :: EX_E_mean_ind(EX_n) = [6.5_RP, 2.375_RP,10.625_RP] !Mean of the environments for the indivduals
  real(kind=RP), parameter :: EX_E_min_ind(EX_n) = (/ 1._RP , 3.75_RP,  6.5_RP/)
  real(kind=RP), parameter :: EX_E_max_ind(EX_n) = (/ 6.5_RP, 9.25_RP, 12._RP /)
  real(kind=RP), parameter :: EX_E_scale = 1.0_RP !Scaling for the environments
  real(kind=RP), parameter :: EX_length = 19.5_RP !Individual length for experiment start
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!                  intake = min(foraging * ValueOfE(E_cat) * SMR_somatic_std, intake_max) ![J timestep-1] depends on somatic weight because predator-prey interactions are determined by length, not influenced by reserves.
                  ...
                  surplus_before_growth = intake - SDA - SMR_THF - foraging_cost !Surplus energy available for GROWTH, storing, reproduction, etc. [J timestep-1]
...
!~                     survival = survival * (5._RP*reserves_new/reserves_max_new)
  ...
  !If forwardOnly = y then read the strategy, fitness and ProbAllE arrays from binary files
  ...
  ind(2,1,:) = ((R_categories_init-1._RP) / real(R_categories-1,RP))*100._RP 
